...
A secure two-page web application for managing products, built with Python and SQLite.
...
- Protected against **XSS attacks** using input validation, HTML escaping, and Content Security Policy headers
- **Rate limiting** on all endpoints (max 5 requests per 60 seconds per IP)
...
```
> Note: Accept the security warning for self-signed certificates, or use `mkcert` for a locally trusted cert.
